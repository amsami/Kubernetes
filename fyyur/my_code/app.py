...
class Venue(db.Model):
    ...
    seking_talent = db.Column(db.Boolean)
...
class Artist(db.Model):
    ...
    facebook_link = db.Column(db.String(120))
    website = db.Column(db.String(120))
    seking_venue = db.Column(db.Boolean)
    seeking_description = db.Column(db.String(500))    
...
def venues():
  ...
  areas=Venue.query.distinct(Venue.city).order_by(Venue.city.desc()).all()
  #For cs in unique_city:
  #  venue=Venue.query.filter_by(=cs)
...
def search_venues():
  ...
  #search_venue_request = request.get_json()['search_term']
  #for venue_name in venue.name
  print(Venue.name)
  #data=
  response={
    "count": 1,
...
def search_artists():
  ...
  data_all=Artist.query.all()
  print ({Artist.name})

  for artist in data_all:
    print ({Artist.name})
    if search_artist_request == Artist.name:
      print (data.name)
